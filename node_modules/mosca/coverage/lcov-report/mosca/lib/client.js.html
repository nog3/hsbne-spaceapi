...
<html lang="en">
<head>
    <title>Code coverage report for mosca/lib/client.js</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">mosca/lib/client.js</span></h1>
    <h2>
        
        Statements: <span class="metric">89.74% <small>(210 / 234)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Branches: <span class="metric">82.02% <small>(73 / 89)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Functions: <span class="metric">93.75% <small>(45 / 48)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Lines: <span class="metric">89.74% <small>(210 / 234)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
    </h2>
    <div class="path"><a href="../../index.html">All files</a> &#187; <a href="index.html">mosca/lib/</a> &#187; client.js</div>
</div>
<div class="body">
...
522
523
524</td><td class="line-coverage"><span class="cline-any cline-yes">1</span>
...
&nbsp;
  client.on("puback", <span class="fstat-no" title="function not covered" >function(packet) {</span>
...
&nbsp;
  client.on("close", function() {
    <span class="missing-if-branch" title="else path not taken"" >E</span>if (!that._closed || <span class="branch-1 cbranch-no" title="branch not covered" >!that._closing)</span> {
      that.onNonDisconnectClose();
    }
  });
};
...
&nbsp;
  ...
  } else <span class="missing-if-branch" title="if path not taken"" >I</span>if (retry === 10) {
<span class="cstat-no" title="statement not covered" >    this.logger.debug(packetToLog, "could not deliver the message");</span>
<span class="cstat-no" title="statement not covered" >    this.connection.emit("error", new Error("client not responding to acks"));</span>
  } else {
&nbsp;
...
Client.prototype._buildForward = function() {
  ...
  this.forward = function(topic, payload, options, subTopic, initialQoS, cb) {
    ...
    } else <span class="missing-if-branch" title="else path not taken"" >E</span>if (options._dedupId &lt;= that._lastDedupId) {
      return;
    }
...
&nbsp;
    var pubQoS = options &amp;&amp; options.qos,
        ...
        qos = Math.min(pubQoS || 0,
                       (subQoS === undefined ? <span class="branch-0 cbranch-no" title="branch not covered" >initialQoS </span>: subQoS) || 0);
&nbsp;
    var packet = {
      topic: topic,
      payload: payload,
...
Client.prototype.unsubscribeMapTo = function(topic) {
  ...
  return function(cb) {
    ...
    handler = (sub &amp;&amp; sub.handler) || <span class="branch-2 cbranch-no" title="branch not covered" >that.forward;</span>
...
<span class="cstat-no" title="statement not covered" >      that.connection.emit("error", err);</span>
<span class="cstat-no" title="statement not covered" >      return;</span>
    }
&nbsp;
    if (!verdict) {
...
<span class="cstat-no" title="statement not covered" >    clearTimeout(this.inflight[packet.messageId].timer);</span>
<span class="cstat-no" title="statement not covered" >    delete this.inflight[packet.messageId];</span>
  } else {
<span class="cstat-no" title="statement not covered" >    logger.warn({ packet: packet }, "no such packet");</span>
  }
};
&nbsp;
/**
...
function calculateGranted(client, packet) {
  return packet.subscriptions.map(function(e) {
    <span class="missing-if-branch" title="if path not taken"" >I</span>if (e.qos === 2) {
<span class="cstat-no" title="statement not covered" >      e.qos = 1;</span>
    }
    <span class="missing-if-branch" title="if path not taken"" >I</span>if (client.subscriptions[e.topic] !== undefined) {
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken"" >E</span>if(!that._closed) {
...
&nbsp;
    ...
    that.connection.on("error", <span class="fstat-no" title="function not covered" >function() {</span>});
...
<div class="footer">
    <div class="meta">Generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Fri Nov 29 2013 16:14:12 GMT+0100 (CET)</div>
</div>
